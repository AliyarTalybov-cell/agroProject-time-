...
// Шапка-предупреждение: скрипт описывает прототип, а не боевую базу.
const OBSOLETE_BANNER = `-- =====================================================================
-- УСТАРЕВШИЙ ФАЙЛ. НЕ ИСПОЛЬЗОВАТЬ ДЛЯ СОЗДАНИЯ БАЗЫ.
--
-- Это схема раннего прототипа. В ней нет многих таблиц боевой базы
-- (новости, уведомления и др.) и заложена старая модель доступа:
-- политики «Allow all … using (true)» без «to authenticated», в том числе
-- на profiles — через неё любой пользователь мог назначить себе роль
-- менеджера. Исправления из supabase/migrations/2026*.sql сюда не входят
-- и поверх этого файла не применяются: скрипт упадёт на отсутствующих
-- таблицах.
--
-- Восстанавливать базу нужно из дампа боевой (pg_dump), а не отсюда.
-- Подробнее — supabase/migrations/README.md.
-- =====================================================================

`;

let out = OBSOLETE_BANNER + '-- AGRO PORTAL DB INIT SCRIPT (dependency-ordered)\n\n';
