...
> **Устарело.** `supabase/schema.sql` и `init-database.sql` описывают ранний
> прототип: в них нет части таблиц и заложена старая, небезопасная модель
> доступа. Боевая база поднимается из дампа (`pg_dump`), а изменения схемы
> идут миграциями из `supabase/migrations/` — см. README в этом каталоге.
> Шаги ниже оставлены для истории.
